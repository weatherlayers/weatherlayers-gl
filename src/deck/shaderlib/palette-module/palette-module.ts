...
function getUniforms(props: Partial<PaletteModuleProps> = {}): PaletteModuleUniforms {
  return {
    [tokens['paletteTexture'] ?? 'paletteTexture']: props.paletteTexture,
    [tokens['paletteBounds'] ?? 'paletteBounds']: props.paletteBounds ?? [0, 0],
    [tokens['paletteColor'] ?? 'paletteColor']: props.paletteColor ? deckColorToGl(props.paletteColor) : [0, 0, 0, 0],
  };
}
...
export const paletteModule = {
  ...
  fs: sourceCode,
  uniformTypes: {
    [tokens['paletteBounds'] ?? 'paletteBounds']: 'vec2<f32>',
    [tokens['paletteColor'] ?? 'paletteColor']: 'vec4<f32>',
  },
  getUniforms,
